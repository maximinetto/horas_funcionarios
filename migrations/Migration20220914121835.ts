...
export class Migration20220914121835 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `actual_balances` (`id` varchar(36) not null, `created_at` datetime null, `updated_at` datetime null, `year` int not null, `total` bigint not null, `official_id` int unsigned null, `type` enum(\'tas\', \'teacher\') not null, primary key (`id`)) default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('create table `hourly_balances_tas` (`id` varchar(36) not null, `created_at` datetime null, `updated_at` datetime null, `year` int not null, `surplus_non_working` bigint not null, `surplus_simple` bigint not null, `surplus_business` bigint not null, `actual_balance_id` varchar(36) null, primary key (`id`)) default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('alter table `hourly_balances_tas` add constraint `hourly_balances_tas_actual_balance_id_foreign` foreign key (`actual_balance_id`) references `actual_balances` (`id`) on update cascade on delete cascade;');
    ...
    this.addSql('alter table `calculation_tas` add constraint `calculation_tas_actual_balance_id_foreign` foreign key (`actual_balance_id`) references `actual_balances` (`id`) on update cascade on delete cascade;');
  }
